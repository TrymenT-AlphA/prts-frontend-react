...
class Point {
  ...
  update(ParticlePolymerizeFlag = true, options, mx, my) {
    ...
    if (effectParticleMode) {
      let finalX =
          (effectParticleMode === "adsorption" ? vx : -vx) * Drag +
          ((this.orx - this.x) * Ease) / 400,
        finalY =
          (effectParticleMode === "adsorption" ? vy : -vy) * Drag +
          ((this.ory - this.y) * Ease) / 400;
      this.spx += finalX;
      this.spy += finalY;
    }
  ...
  changePos(newX, newY, colorVal) {
    if (newX) this.orx = this.nx = newX;
    if (newY) this.ory = this.ny = newY;
    if (colorVal) {
...
class DameDaneParticle {
  ...
  constructor(canvas, options, callback) {
    ...
    this.IMG.onload = () => {
      ...
      if (typeof h === "number") this.ImgH = h;
        ...
        this.ImgH = Math.floor(this.ImgW * (this.IMG.height / this.IMG.width));
  ...
  _InitParticle = (ImgData, rebuildParticle = false) => {
    ...
    let { spacing, size, validColor, cancelParticleAnimation } = this.options,
      proportion = window.innerHeight / window.outerHeight;
    ...
    for (var h = 0; h < imgH; h += gap) {
      for (var w = 0; w < imgW; w += gap) {
        ...
        if (
          (validColor.invert &&
            (val <= validColor.min || val >= validColor.max)) ||
  ...
  _Draw2Canvas = () => {
    this.hasDraw = true;
    const w = this.canvasEle.width,
      h = this.canvasEle.height;
    this.ctx.clearRect(0, 0, w, h);
    this.PointArr.forEach(
      /** @param {Point} point */
      (point) => {
        point.update(
          this.ParticlePolymerizeFlag,
          this.options,
          this.mx,
          this.my
        );
  ...
  ParticlePolymerize(flag) {
    ...
    this.PointArr.forEach(
      /** @param {Point} point */
      (point) => {
        point.nx = this.ParticlePolymerizeFlag
          ? point.orx
          : Math.random() * this.canvasEle.width;
        point.ny = this.ParticlePolymerizeFlag
          ? point.ory
          : Math.random() * this.canvasEle.height;
      }
    );
  }
...
export default DameDaneParticle;
